...
namespace hypeJunction\Inbox\Tests;

use PHPUnit\Framework\TestCase;

/**
 * Auto-generated migration regression guard for plugin "hypeinbox".
 *
...
final class MigrationRegressionTest extends TestCase
{
    ...
    public function testNoRemovedSymbols(): void
    {
        ...
        foreach (self::files('', 'php') as $file) {
            ...
            foreach (explode("\n", (string) file_get_contents($file)) as $n => $line) {
                // Skip comment-only lines: migrated code often documents a
                // removed symbol in a // TODO / docblock (e.g. "elgg_get_metastring_id()
                // removed …"), which is NOT a live call. A real call sits on a
                // code line. This avoids false positives on migration notes.
                $trimmed = ltrim($line);
    ...
    public function testManifestUsesStringLiterals(): void
    {
        ...
        // a class CONSTANT (MyClass::SUBTYPE) in the entities block autoloads
        // the class and fatals. A bare `::class` is a compile-time literal that
        // never autoloads, so it is SAFE — this plugin uses
        // `\hypeJunction\Inbox\Message::class` and the manifest parses fine
        // (BootstrapTest::testMessageEntitySubtypeRegistered is green). Only
        // flag class-constant refs here.
        $src = self::manifestSource();
